...
Novità: Integrazione Fallback su API FMP e sistema di Auto-Update da GitHub.
...
class GitHubUpdateManager:
    """Gestisce il controllo degli aggiornamenti tramite le API di GitHub Releases."""

    @staticmethod
    def check_for_updates(current_version: str, repo_path: str) -> Tuple[bool, str, str]:
        """
        Interroga GitHub per verificare se esiste una release più recente.

        Args:
            current_version: La versione attuale del software (es. '0.0.20').
            repo_path: Il percorso della repository su GitHub (es. 'user/repo').

        Returns:
            Tuple contenente: (Update disponibile (bool), Nuova Versione (str), URL Download (str)).

        Raises:
            ValueError: Se la richiesta all'API fallisce.
        """
        ...
        try:
            ...
            latest_tag: str = data.get('tag_name', '').replace('v', '') # Rimuove un'eventuale 'v' iniziale
            html_url: str = data.get('html_url', '')

            if not latest_tag:
                return False, current_version, ""

            # Semplice comparazione di stringhe per il versioning (es. '0.0.21' > '0.0.20')
            # Per versioning complessi andrebbe usata la libreria 'packaging'
            update_available = latest_tag > current_version.replace('v', '')
...
class DataFormatter:
    """Gestisce la conversione bidirezionale tra numeri e stringhe formattate."""

    @staticmethod
    def parse_to_float(value_str: str) -> float:
        """Converte stringhe con suffissi (es. '5,5M') in float numerici."""
        ...
        try:
            ...
            raise ValueError(f"Formato non valido: '{value_str}'.")

    @staticmethod
    def format_to_string(value: float) -> str:
        """Formatta un float in una stringa leggibile (K, M, B)."""
...
class FinancialEvaluator:
    """Implementa l'algoritmo di scoring per valutare l'azienda."""

    @staticmethod
    def evaluate(ey: float, roic: float, ev_ebitda: float) -> Tuple[float, str, str, Dict[str, Dict[str, str]]]:
        """Calcola voto, raccomandazione e restituisce le descrizioni contestuali."""
...
class FinancialCalculator:
    """Gestisce la validazione e il calcolo matematico delle metriche."""

    @staticmethod
    def validate_inputs(data: Dict[str, str]) -> Dict[str, float]:
        """Valida e converte un dizionario di stringhe in valori numerici."""
        validated_data: Dict[str, float] = {}
        for key, value_str in data.items():
            validated_data[key] = DataFormatter.parse_to_float(value_str)
        return validated_data

    @staticmethod
    def calculate_metrics(data: Dict[str, float]) -> Dict[str, Union[float, str]]:
        """Calcola gli indicatori gestendo la divisione per zero."""
...
class TickerSearcher:
    """Interroga l'API di Yahoo Finance per cercare i ticker in base al nome."""

    @staticmethod
    def search_by_name(query: str) -> List[Tuple[str, str, str]]:
        """Ricerca corrispondenze tramite nome azienda."""
...
class FinancialDataFetcher:
    """Gestisce l'estrazione dati. Usa yfinance come primario, FMP come fallback."""

    def __init__(self, fmp_api_key: str = "") -> None:
        self.fmp_api_key = fmp_api_key

    def fetch_fundamental_data(self, ticker_symbol: str) -> Dict[str, Any]:
        """Scarica i dati tentando prima con Yahoo Finance. Se fallisce, usa FMP."""
        ...
        try:
            ...
            if self.fmp_api_key:
                try:
                    return self._fetch_from_fmp(ticker_symbol)
                except Exception as fmp_error:
                    raise ValueError(f"Sia Yahoo Finance che FMP hanno fallito.\nYahoo: {yf_error}\nFMP: {fmp_error}")
            else:
                raise ValueError(f"Errore Yahoo Finance (nessuna API configurata): {str(yf_error)}")
    ...
    def _fetch_from_yahoo(self, ticker_symbol: str) -> Dict[str, Any]:
        ...
        if ev == 0.0 and ebitda == 0.0:
             raise ValueError("Dati EV/EBITDA non forniti da Yahoo per questo titolo.")
    ...
    def _fetch_from_fmp(self, ticker_symbol: str) -> Dict[str, Any]:
        """Scarica i dati utilizzando Financial Modeling Prep API come fallback."""
        ...
        try:
            ...
            tax_rate = 0.21
            if inc.get('incomeBeforeTax', 0) > 0:
                tax_rate = inc.get('incomeTaxExpense', 0) / inc.get('incomeBeforeTax', 1)
            ...
            raise ValueError(f"Errore connessione API FMP: {str(e)}")
...
class UpdateCheckWorker(QThread):
    """Esegue in asincrono il check della versione su GitHub per non bloccare la UI."""
...
class FetchWorker(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            self.error.emit(f"Errore di sistema: {str(e)}")
...
class GuideDialog(QDialog):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        <ul>
            ...
            <li><b>Perché funziona:</b> Identifica le aziende con un "Moat" elevato. Un ROIC superiore al WACC è il principale motore della creazione di valore.</li>
        </ul>

        <h2 style='color: #2980b9;'>3. Enterprise Value to EBITDA (EV/EBITDA)</h2>
        <p>Moltiplo più affidabile del P/E per confrontare aziende con diverse strutture di capitale.</p>
        <ul>
            <li><b>Perché funziona:</b> L'EBITDA è meno soggetto a manipolazioni contabili e l'EV neutralizza le differenze di leva finanziaria.</li>
        </ul>
        """
...
class FmpSetupDialog(QDialog):
    """Dialogo per il setup dell'API Key FMP al primo avvio."""
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
    ...
    def _init_ui(self) -> None:
        ...
        lbl_desc = QLabel(
            ...
            "Puoi inserire gratuitamente una API Key di <b>Financial Modeling Prep (FMP)</b><br>"
            "che farà da piano di riserva (Fallback) automatico in caso di errori.<br><br>"
            "<i>(Piano FREE: 250 chiamate al giorno, nessuna carta richiesta)</i>"
        )
...
class TickerSearchDialog(QDialog):
    """Dialogo per la selezione manuale del ticker se la ricerca produce risultati multipli."""
...
class InfoDialog(QDialog):
    """Finestra di dialogo 'Info' obbligatoria, isolata in classe dedicata."""
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
...
class MainWindow(QMainWindow):
    """Finestra principale dell'applicazione, gestisce input utente e visualizzazione dati."""
    def __init__(self) -> None:
        ...
        # Check asincroni all'avvio: FMP setup e Aggiornamenti GitHub
    ...
    def _reset_api_key(self) -> None:
        ...
        QMessageBox.information(self, "API Resettata", "Impostazioni ripristinate. Al prossimo riavvio ti verrà richiesta la chiave.")

    def _check_for_updates(self, silent: bool = True) -> None:
        """Avvia il worker per cercare aggiornamenti su GitHub."""
        if not silent:
    ...
    def _on_update_checked(self, update_available: bool, new_version: str, download_url: str, silent: bool) -> None:
        if update_available:
            ...
            reply = QMessageBox.question(
                ...
                f"È disponibile una nuova versione di {APP_NAME} (<b>v{new_version}</b>).<br><br>"
                "Vuoi scaricare il nuovo pacchetto di installazione?",
    ...
    def _on_search_requested(self) -> None:
        ...
        self.search_worker.finished.connect(lambda results: self._on_search_success(results, query))
